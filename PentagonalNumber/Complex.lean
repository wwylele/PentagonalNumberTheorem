...
theorem γ_bound (N n : ℕ) {x : ℂ} (hx : ‖x‖ < 1) :
    ‖Pentagonal.γ N n x‖ ≤ ‖x‖ ^ ((N + 1) * n) * ∏' i, (1 + ‖x‖ ^ i) := by
  unfold Pentagonal.γ
...
theorem summable_γ_complex (N : ℕ) {x : ℂ} (hx : ‖x‖ < 1) : Summable (Pentagonal.γ N · x) := by
  rw [← summable_norm_iff]
  apply Summable.of_nonneg_of_le (by simp) (γ_bound N · hx)
...
theorem tsum_γ_bound (N : ℕ) {x : ℂ} (hx : ‖x‖ < 1) :
    ‖∑' n, Pentagonal.γ N n x‖ ≤ (1 - ‖x‖)⁻¹ * ∏' i, (1 + ‖x‖ ^ i) := by
...
/-- **Pentagonal number theorem** for natural numbers, summation over integers, opposite order.
...
/-- **Pentagonal number theorem** for natural numbers, summation over integers, classic order.
